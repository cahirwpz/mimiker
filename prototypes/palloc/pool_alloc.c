...
typedef struct pool_item {
  uint64_t pi_guard_number; // 0xcafebabecafebabe by default, normally isn't
...
pool_slab_t *create_slab(size_t size, void (*constructor)(void *, size_t)) {
  ...
  for (int i = 0; i < slab->ph_ntotal; i++) {
    pool_item_t *curr_pi = (pool_item_t *)((uint64_t)slab + (slab->ph_start) +
...
void destroy_slab(pool_slab_t *slab, pool_t *pool) {
  ...
  for (int i = 0; i < slab->ph_ntotal; i++) {
    pool_item_t *curr_pi = (pool_item_t *)slab + (slab->ph_start) +
                           i * ((pool->pp_itemsize) + sizeof(pool_item_t));
...
void *slab_alloc(pool_slab_t *slab, size_t size) {
  ...
  pool_item_t *found_pi = (pool_item_t *)((uint64_t)slab + (slab->ph_start) +
                                          i * (size + sizeof(pool_item_t)));
