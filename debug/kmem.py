...
class Kmem(gdb.Command, utils.OneArgAutoCompleteMixin):
    ...
    kmem without arguments prints a short summary
    """
    ...
    def get_pool_names(self):
        ...
        for line in lines:
            line = line.split()
            if (len(line) == 2) and (line[0] == 'kmem_pool_t'):
                varname = line[1][1:-1]
                result.append(varname)
            if (len(line) == 3) and (line[0] == 'static') \
               and (line[1] == 'kmem_pool_t'):
                varname = line[2][1:-1]
                result.append(varname)
    ...
    def add_row_to_pool_table(self, table, pool):
        ...
        name = str('%s' % pool)
        descr = str(pool_val['mp_desc'])
        pused = str(pool_val['mp_pages_used'])
        pmax = str(pool_val['mp_pages_max'])
        table.append([name, descr, pused, pmax])
        table
