...
/*   int (*cpystr)(const void* saddr, void* daddr, size_t len, size_t*
 * lencopied); */
/* } copy_ops_t; */
...
/* int copy_ptrs(char* data, void** src_ptr_array, size_t size, size_t*
 * ptrs_copied, copy_ops_t* ops) */
/* { */
...
/* int copy_strings(char* data, char** str_ptrs, size_t size, size_t*
 * size_copied, copy_ops_t* ops) */
/* { */
/*   char* strings = data; */
/*   size_t str_size; */

/*   for (size_t i = 0; str_ptrs[i] != NULL; i++) { */
/*     int result = ops->cpystr(str_ptrs[i], strings, size - *size_copied,
 * &str_size); */
/*     if (result < 0) */
...
/* int copy_exec_args(char* data, size_t size, char** argv, char** envp, size_t*
 * size_copied, copy_ops_t* ops) */
/* { */
/*   size_t ptrs_copied; */
/*   size_t bytes_copied; */
...
/*   result = copy_strings(data, argv_ptrs, bytes_remaining, &bytes_copied,
 * ops); */
/*   if (result != 0) */
/*     return result; */
...
/*   result = copy_strings(data, envp_ptrs, bytes_remaining, &bytes_copied,
 * ops); */
/*   if (result != 0) */
/*     return result; */
/*   bytes_remaining -= bytes_copied; */
...
// int cpy_strarray_ptrs(char *data, const char** strarray, size_t size, size_t
// *strptrs_copied, copy_ops_t* ops)
// {
