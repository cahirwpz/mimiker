...
static int paging_on_demand_and_memory_protection_demo(void) {
  ...
  /* data segment */
  {
    ...
    vm_segment_t *seg =
      vm_segment_alloc(obj, start, end, VM_PROT_READ | VM_PROT_WRITE, 0);
    n = vm_map_insert(umap, seg, VM_FIXED);
    assert(n == 0);
  }
  ...
  {
    vm_object_t *obj = vm_object_alloc(VM_DUMMY);
    vm_segment_t *seg = vm_segment_alloc(obj, end, post_end, VM_PROT_NONE, 0);
    n = vm_map_insert(umap, seg, VM_FIXED);
    assert(n == 0);
  }
...
static int findspace_demo(void) {
  ...
  seg = vm_segment_alloc(NULL, addr1, addr2, VM_PROT_NONE, 0);
  n = vm_map_insert(umap, seg, VM_FIXED);
  assert(n == 0);

  seg = vm_segment_alloc(NULL, addr3, addr4, VM_PROT_NONE, 0);
  n = vm_map_insert(umap, seg, VM_FIXED);
  assert(n == 0);
  ...
  seg = vm_segment_alloc(NULL, addr2, addr2 + 0x5000, VM_PROT_NONE, 0);
  n = vm_map_insert(umap, seg, VM_FIXED);
  assert(n == 0);
