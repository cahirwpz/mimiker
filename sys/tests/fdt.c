...
static const void *fdt = (const void *)__malta_dtb_start;

static int test_fdt_get_path(void) {
  const char *path = "/root/cbus@1f000000/serial@900";
  char buf[BUF_SIZE];
...
static int test_fdt_get_name(void) {
  const char *path = "/root/pci@1be00000/isa@0,0/serial@3f8";
  const char *name = "serial@3f8";
  int len;
...
static int test_fdt_subnode(void) {
  const char *path = "/root/pci@1be00000/isa@0,0";
  const char *name_first = "rtc@70";
  const char *name_next = "atkbdc@60";
  const char *retval;
  int offset;
  int len;
