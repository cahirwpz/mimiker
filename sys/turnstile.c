...
typedef struct turnstile {
  union {
    LIST_ENTRY(turnstile) ts_hash_link; /* link on turnstile chain */
...
static turnstile_t *turnstile_lookup(void *wchan) {
  ...
  turnstile_t *ts;
  LIST_FOREACH(ts, &tc->tc_turnstiles, ts_hash_link) {
...
void turnstile_wait(void *wchan, thread_t *owner, const void *waitpt) {
  ...
  if (ts == td->td_turnstile) {
    LIST_INSERT_HEAD(&tc->tc_turnstiles, ts, ts_hash_link);
...
static void turnstile_free_return(turnstile_t *ts) {
  ...
  TAILQ_FOREACH (td, &ts->ts_blocked, td_lockq) {
    ...
    if (LIST_EMPTY(&ts->ts_free)) {
      ...
      ts_for_td = ts;
      LIST_REMOVE(ts_for_td, ts_hash_link);
    } else {
