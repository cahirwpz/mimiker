...
void thread_entry_setup(thread_t *td, entry_fn_t target, void *arg) {
  ...
  exc_frame_t *uframe = kstack_alloc_s(stack_bottom(stk), exc_frame_t);
  exc_frame_t *kframe = kstack_alloc_s(uframe, cpu_exc_frame_t);
  ctx_t *kctx = kstack_alloc_s(kframe, ctx_t);
