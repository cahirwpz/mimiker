...
static kft_event_t make_event(uint8_t type, tid_t thread, uint64_t timestamp,
                              uint64_t pc) {
...
static __no_profile void add_event(uint8_t type, uintptr_t pc) {
  uint64_t time = READ_SPECIALREG(cntpct_el0);
  tid_t thread = thread_self()->td_tid;

  if ((char *)pc < __kernel_start || __kernel_end < (char *)pc)
    return;
