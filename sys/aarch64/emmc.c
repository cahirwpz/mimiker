...
typedef struct emmc_state {
    ...
    spin_t slock;
    uint32_t intr_mask; /* Mask these flags during an interrupt */
    uint32_t intr_response;
    uint64_t sd_scr[2];
    uint64_t sd_ocr;
    uint64_t sd_rca;
    uint64_t sd_err;
...
static int emmc_intr_wait(device_t * dev, uint32_t mask) {
...
int emmc_read_block(device_t *dev, uint32_t lba, unsigned char *buffer,
  ...
  while (c < num) {
    ...
    if ((r = emmc_intr_wait(dev, INT_READ_RDY))) {
...
int emmc_write_block(device_t *dev, uint32_t lba, const uint8_t *buffer,
  ...
  while (c < num) {
    if (!(state->sd_scr[0] & SCR_SUPP_CCS)) {
      ...
      if ((r = emmc_intr_wait(dev, INT_WRITE_RDY))) {
...
static int emmc_init(device_t *dev) {
  ...
  if (state->sd_err)
    return state->sd_err;
  if (emmc_intr_wait(dev, INT_READ_RDY))
    return SD_TIMEOUT;
