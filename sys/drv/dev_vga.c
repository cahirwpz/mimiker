...
static int videomode_write(vnode_t *v, uio_t *uio) {
  vga_device_t *vga = devfs_node_data_old(v);
...
void dev_vga_install(vga_device_t *vga) {
  ...
  devfs_makedev_old(vga_root, "fb", &framebuffer_vnodeops, vga, NULL);
  devfs_makedev_old(vga_root, "palette", &palette_vnodeops, vga, NULL);
  devfs_makedev_old(vga_root, "videomode", &videomode_vnodeops, vga, NULL);
}
