...
static int pl011_attach(device_t *dev) {
  ...
  if ((err = pic_setup_intr(dev, intr, uart_intr, NULL, dev, "PL011 UART")))
  ...
  /* Disable UART0. */
  bus_write_4(regs, PL011COM_CR, 0);
  /* Clear pending interrupts. */
  bus_write_4(regs, PL011COM_ICR, PL011_INT_ALLMASK);
  ...
  bus_write_4(regs, PL011COM_IBRD, 2);
  bus_write_4(regs, PL011COM_FBRD, 0xb);

  /* Enable FIFO & 8 bit data transmission (1 stop bit, no parity). */
  bus_write_4(regs, PL011COM_LCRH, PL01X_LCR_FEN | PL01X_LCR_8BITS);

  /* Mask all interrupts. */
  bus_write_4(regs, PL011COM_IMSC, PL011_INT_ALLMASK);

  /* Enable UART0, receive & transfer part of UART. */
  bus_write_4(regs, PL011COM_CR, PL01X_CR_UARTEN | PL011_CR_TXE | PL011_CR_RXE);

  /* Enable interrupt. */
  bus_write_4(regs, PL011COM_IMSC, PL011_INT_RX);
