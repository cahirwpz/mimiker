...
static vm_page_t *dummy_pager_fault(uvm_object_t *obj, off_t offset) {
  return NULL;
}

static vm_page_t *anon_pager_fault(uvm_object_t *obj, off_t offset) {
  assert(obj != NULL);

  vm_page_t *new_pg = vm_page_alloc(1);
  pmap_zero_page(new_pg);
  uvm_object_add_page(obj, offset, new_pg);
  return new_pg;
}
