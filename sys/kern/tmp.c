...
static inline size_t compute_pg_size(paddr_t pa) {
  /* Take the highest number of the form 2^k * `PAGESIZE`
   * that divides the physical address of the page. */
...
void init_vm_page(void) {
  ...
  TAILQ_FOREACH (seg, &seglist, seglink) {
    ...
    for (unsigned i = 0; i < seg->npages; i++) {
      ...
      size_t size = compute_pg_size(pa);
	 ...
	 */
        size = compute_pg_size(seg->end ^ pa);
      }
      page->paddr = pa;
      page->size = size;
