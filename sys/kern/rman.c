...
static KMALLOC_DEFINE(M_RES, "rman resources");

struct rman_resource {
  ...
  res_flags_t flags;               /* or'ed RF_* values */
  TAILQ_ENTRY(rman_resource) link; /* link on resource manager list */
};

bus_size_t resource_size(resource_t *r) {
  rman_resource_t *res = r->r_res;
  return res->end - res->start + 1;
}

rman_addr_t resource_start(resource_t *r) {
  return r->r_res->start;
}

bool resource_active(resource_t *r) {
  return r->r_res->flags & RF_ACTIVE;
}
...
static rman_resource_t *resource_alloc(rman_t *rm, rman_addr_t start,
                                       rman_addr_t end, res_flags_t flags) {
  rman_resource_t *r = kmalloc(M_RES, sizeof(rman_resource_t), M_ZERO);
  r->rman = rm;
  r->start = start;
  r->end = end;
  r->flags = flags;
...
static int r_active(rman_resource_t *r) {
  return r->flags & RF_ACTIVE;
}

static int r_reserved(rman_resource_t *r) {
  return r->flags & RF_RESERVED;
}

static int r_canmerge(rman_resource_t *r, rman_resource_t *next) {
...
void rman_manage_region(rman_t *rm, rman_addr_t start, size_t size) {
  ...
  rman_resource_t *reg = resource_alloc(rm, start, end, RF_RESERVED);

  WITH_MTX_LOCK (&rm->rm_lock) {
    rman_resource_t *first = TAILQ_FIRST(&rm->rm_resources);
    if (first == NULL || end < first->start) {
      TAILQ_INSERT_HEAD(&rm->rm_resources, reg, link);
      goto done;
    }

    rman_resource_t *last = TAILQ_LAST(&rm->rm_resources, rman_res_list);
    if (start > last->end) {
      TAILQ_INSERT_TAIL(&rm->rm_resources, reg, link);
      goto done;
    }

    rman_resource_t *next;
    for (rman_resource_t *r = first; r != last; r = next) {
      next = TAILQ_NEXT(r, link);
      if (r->end < start && end < next->start) {
        TAILQ_INSERT_AFTER(&rm->rm_resources, r, reg, link);
        goto done;
      }
    }
...
void rman_fini(rman_t *rm) {
  ...
  while (!TAILQ_EMPTY(&rm->rm_resources)) {
    rman_resource_t *r = TAILQ_FIRST(&rm->rm_resources);
    assert(!r_reserved(r)); /* can't free resource that's in use */
    TAILQ_REMOVE(&rm->rm_resources, r, link);
    kfree(M_RES, r);
  }
...
rman_resource_t *rman_reserve_resource(rman_t *rm, rman_addr_t start,
  ...
  rman_resource_t *r;
  TAILQ_FOREACH (r, &rm->rm_resources, link) {
    ...
    if (r->start < new_start) {
      rman_resource_t *prev =
        resource_alloc(rm, r->start, new_start - 1, r->flags);
      TAILQ_INSERT_BEFORE(r, prev, link);
      r->start = new_start;
    }

    if (r->end > new_end) {
      rman_resource_t *next = resource_alloc(rm, new_end + 1, r->end, r->flags);
      TAILQ_INSERT_AFTER(&rm->rm_resources, r, next, link);
      r->end = new_end;
    }
...
void rman_release_resource(rman_resource_t *r) {
   ...
   */
  rman_resource_t *prev = TAILQ_PREV(r, rman_res_list, link);
  ...
  rman_resource_t *next = TAILQ_NEXT(r, link);
  if (next && r_canmerge(r, next)) {
    /* Merge next region with ours. */
    r->end = next->end;
    TAILQ_REMOVE(&rm->rm_resources, next, link);
    kfree(M_RES, next);
  }
