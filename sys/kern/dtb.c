...
int dtb_soc_node(void) {
  return dtb_offset("soc");
}

int dtb_bus_child_node(device_t *bus, const char *name) {
  int node = fdt_subnode_offset(_dtb_root, bus->node, name);
  if (node < 0)
    panic("Bus %p doesn't have %s child node!", bus, name);
  return node;
}

const char *dtb_dev_compatible(device_t *dev) {
  return dtb_str(dev->node, "compatible");
}

unsigned long dtb_dev_cell(device_t *dev, const char *name) {
  return dtb_cell(dev->node, name);
}

void dtb_dev_reg(device_t *dev, unsigned long *addr_p, unsigned long *size_p) {
  int parent = dev->parent->node;
  dtb_reg(parent, dev->node, addr_p, size_p);
}
