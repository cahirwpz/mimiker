...
static resource_t *find_resource(rman_t *rm, rman_res_t start, rman_res_t end,
                                 rman_res_t count) {
  for (resource_t *resource = rm->resources.lh_first; resource != NULL;
    ...
    rman_res_t s = max(start, resource->r_start);
    rman_res_t e = min(end, resource->r_end);

    rman_res_t len = s - e + 1;
...
static resource_t *cut_resource(resource_t *resource, rman_res_t where) {
...
// after allocation
static resource_t *split_resource(resource_t *resource, rman_res_t start,
