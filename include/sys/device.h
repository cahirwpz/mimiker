...
typedef struct dev_mmio dev_mmio_t;
...
typedef SLIST_HEAD(, dev_mmio) dev_mmio_list_t;
...
struct device {
  ...
  dev_mmio_list_t mmio_list; /* memory mapped I/O assigned to the device */
};
...
struct dev_mmio {
  SLIST_ENTRY(dev_mmio) link;    /* link on resource list */
  ...
  dev_mmio_flags_t flags;        /* flags describing the resource */
  unsigned id;                   /* device-relative resource ID */
};

/* TODO: remove it. */
#define DEV_MMIO_DECLARE(name) extern dev_mmio_t name[1]

/*! \brief Calculate resource size. */
static inline bus_size_t dev_mmio_size(dev_mmio_t *mmio) {
  return mmio->end - mmio->start;
}
