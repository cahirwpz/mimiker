...
typedef SLIST_HEAD(, interrupt) intr_list_t;
typedef SLIST_HEAD(, mmio) mmio_list_t;
...
struct device {
  ...
  intr_list_t intr_list; /* interrupts generated by the device */
  mmio_list_t mmio_list; /* memory blocks assigned to the device */
};
...
typedef enum {
  ...
  MMIOF_PREFETCHABLE = 1,
} mmio_flags_t;

struct interrupt {
  SLIST_ENTRY(interrupt) link;
  fdt_intr_t *fdt_intr;
  device_t *pic;
...
static inline bus_size_t mmio_size(mmio_t *mmio) {
