 ...
 * then are collected by the linker.
 * */
...
/* Add \sym entry to the linker set where \sym type needs to be equal to
 * linker set's \ptype` */
...
/* Define a new linker set with \set name and entries of type \ptype */
#define SET_DECLARE(set, ptype)                                                \
  extern ptype *__CONCAT(__start_set_, set);                                   \
  extern ptype *__CONCAT(__stop_set_, set)

/* Returns the beggining of the linker set of type **t
 * where t is the linker set's \ptype
 * */
