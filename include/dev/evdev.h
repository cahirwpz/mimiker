...
/*
 * Set the name of a evdev device. This data is only used by the user-space.
 */
void evdev_set_name(evdev_dev_t *evdev, const char *name);

/*
 * Set the input device ID (bus, vendor, product, version). This data is only
...
/*
 * Mark the given event type (EV_SYN, EV_KEY, etc.) as supported. This data is
 * only used by the user-space.
 */
void evdev_support_event(evdev_dev_t *evdev, uint16_t type);

/*
 * Mark the given key code as supported. This data is only used by the
 * user-space.
 */
