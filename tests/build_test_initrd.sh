...
function gen_file()
{
    ...
    name=$2
    mkdir -p $path
    echo "These are contents of file $name in directory $path: " > $path/$name
}
...
gen_file "initrd/very/very/very/very/very/very/very/very/very/deep/directory" "deep_inside"
...
cd initrd 
find -depth -print | cpio --format=crc -o > ../test_initrd.cpio
cd ..
