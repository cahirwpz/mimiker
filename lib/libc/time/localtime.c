...
long tzgetgmtoff(const timezone_t sp, int isdst) {
  int i;
  long l = -1;
  for (i = 0; i < sp->typecnt; ++i) {
    const struct ttinfo *const ttisp = &sp->ttis[i];
...
static int tzloadbody(char const *name, struct state *sp, bool doextend,
  ...
  for (stored = 4; stored <= 8; stored *= 2) {
    ...
    if (!(0 <= leapcnt && leapcnt < TZ_MAX_LEAPS && 0 <= typecnt &&
          typecnt < TZ_MAX_TYPES && 0 <= timecnt && timecnt < TZ_MAX_TIMES &&
          0 <= charcnt && charcnt < TZ_MAX_CHARS &&
          (ttisstdcnt == typecnt || ttisstdcnt == 0) &&
          (ttisgmtcnt == typecnt || ttisgmtcnt == 0)))
      return EINVAL;
    if ((size_t)nread < (tzheadsize               /* struct tzhead */
                         + timecnt * stored       /* ats */
                         + timecnt                /* types */
                         + typecnt * 6            /* ttinfos */
                         + charcnt                /* chars */
    ...
    for (i = 0; i < sp->timecnt; ++i) {
      ...
      if (sp->types[i]) {
        time_t attime =
          ((TYPE_SIGNED(time_t) ? at < TIME_T_MIN : at < 0) ? TIME_T_MIN
                                                            : (time_t)at);
    ...
    for (i = 0; i < sp->leapcnt; ++i) {
      ...
      if (tr <= TIME_T_MAX) {
        ...
        sp->lsis[leapcnt].ls_trans = (time_t)(prevtr = tr);
    ...
    if (tzparse(&up->buf[1], ts, false)) {
      ...
      if (gotabbr == ts->typecnt) {
        ...
        for (i = 0; i < ts->timecnt; i++)
          if (sp->timecnt == 0 || sp->ats[sp->timecnt - 1] < ts->ats[i])
            break;
...
static bool typesequiv(const struct state *sp, int a, int b) {
  ...
  if (sp == NULL || a < 0 || a >= sp->typecnt || b < 0 || b >= sp->typecnt)
    ...
    result =
      ap->tt_gmtoff == bp->tt_gmtoff && ap->tt_isdst == bp->tt_isdst &&
      ap->tt_ttisstd == bp->tt_ttisstd && ap->tt_ttisgmt == bp->tt_ttisgmt &&
...
static ATTRIBUTE_PURE const char *getzname(const char *strp) {
  char c;
...
static ATTRIBUTE_PURE const char *getqzname(const char *strp, const int delim) {
  int c;
...
static const char *getnum(const char *strp, int *const nump, const int min,
                          const int max) {
  char c;
  int num;
...
static int_fast32_t transtime(const int year, const struct rule *const rulep,
...
static bool tzparse(const char *name, struct state *sp, bool lastditch) {
  const char *stdname;
  const char *dstname;
  size_t stdlen;
  size_t dstlen;
        ...
        if (sp->ttis[j].tt_ttisgmt) {
          ...
          */
          if (isdst && !sp->ttis[j].tt_ttisstd) {
            sp->ats[i] += (time_t)(dstoffset - theirdstoffset);
          } else {
            sp->ats[i] += (time_t)(stdoffset - theirstdoffset);
          }
        }
...
/*ARGSUSED*/
static struct tm *localsub(struct state const *sp, time_t const *timep,
                           int_fast32_t setname, struct tm *const tmp) {
  const struct ttinfo *ttisp;
  int i;
  struct tm *result;
...
struct tm *localtime_rz(timezone_t sp, time_t const *timep, struct tm *tmp) {
  return localsub(sp, timep, 0, tmp);
}

#endif

static struct tm *localtime_tzset(time_t const *timep, struct tm *tmp,
                                  bool setname) {
  rwlock_wrlock(&__lcl_lock);
...
static struct tm *gmtsub(struct state const *sp, const time_t *timep,
  ...
  */
  if (result)
    result->TM_ZONE =
      offset ? __UNCONST(wildabbr) : gmtptr ? gmtptr->chars : __UNCONST(gmt);
#endif /* defined TM_ZONE */
  return result;
}
...
static int ATTRIBUTE_PURE leaps_thru_end_of(const int y) {
...
static bool increment_overflow_time(time_t *tp, int_fast32_t j) {
  ...
  */
  if (!(j < 0 ? (TYPE_SIGNED(time_t) ? TIME_T_MIN - j <= *tp : -1 - j < *tp)
              : *tp <= TIME_T_MAX - j))
    return true;
  *tp += j;
  return false;
...
static bool normalize_overflow(int *const tensptr, int *const unitsptr,
                               const int base) {
  int tensdelta;
...
static bool normalize_overflow32(int_fast32_t *tensptr, int *unitsptr,
                                 int base) {
  int tensdelta;
  ...
  */
  ...
  for (;;) {
...
#ifdef NO_ERROR_IN_DST_GAP
      if (ilo != lo && lo - 1 == hi && yourtm.tm_isdst < 0 && do_norm_secs) {
        for (i = sp->typecnt - 1; i >= 0; --i) {
          for (j = sp->typecnt - 1; j >= 0; --j) {
            time_t off;
            if (sp->ttis[j].tt_isdst == sp->ttis[i].tt_isdst)
              continue;
            off = sp->ttis[j].tt_gmtoff - sp->ttis[i].tt_gmtoff;
            yourtm.tm_sec += off < 0 ? -off : off;
            goto again;
          }
        }
      }
    ...
    if (mytm.TM_GMTOFF != yourtm.TM_GMTOFF &&
      ...
      time_t altt = t;
      int_fast32_t diff = (int_fast32_t)(mytm.TM_GMTOFF - yourtm.TM_GMTOFF);
      if (!increment_overflow_time(&altt, diff)) {
        struct tm alttm;
        if (!funcp(sp, &altt, offset, &alttm) &&
            alttm.tm_isdst == mytm.tm_isdst &&
            alttm.TM_GMTOFF == yourtm.TM_GMTOFF && tmcomp(&alttm, &yourtm)) {
          t = altt;
          mytm = alttm;
        }
      }
    ...
    for (i = sp->typecnt - 1; i >= 0; --i) {
      ...
      for (j = sp->typecnt - 1; j >= 0; --j) {
        if (sp->ttis[j].tt_isdst == yourtm.tm_isdst)
          continue;
        newt = (time_t)(t + sp->ttis[j].tt_gmtoff - sp->ttis[i].tt_gmtoff);
  ...
  for (sameind = 0; sameind < nseen; ++sameind) {
    ...
    for (otherind = 0; otherind < nseen; ++otherind) {
      ...
      if (sp->ttis[otheri].tt_isdst == tmp->tm_isdst)
        continue;
      tmp->tm_sec +=
        (int)(sp->ttis[otheri].tt_gmtoff - sp->ttis[samei].tt_gmtoff);
...
static time_t mktime_tzname(timezone_t sp, struct tm *tmp, bool setname) {
  if (sp)
...
time_t mktime_z(timezone_t sp, struct tm *const tmp) {
...
static int_fast64_t leapcorr(const timezone_t sp, time_t t) {
...
NETBSD_INSPIRED_EXTERN time_t time2posix_z(timezone_t sp, time_t t) {
...
NETBSD_INSPIRED_EXTERN time_t posix2time_z(timezone_t sp, time_t t) {
  time_t x;
  time_t y;
