...
#ifndef VAX
static CONST int fivesbits[] = {0,  3,  5,  7,  10, 12, 14, 17, 19, 21, 24, 26,
                                28, 31, 33, 35, 38, 40, 42, 45, 47, 49, 52};
#endif
...
static int rvOK
#ifdef KR_headers
  (d, fpi, expt, bits, exact, rd, irv) U *d;
CONST FPI *fpi;
Long *expt;
ULong *bits;
int exact, rd, *irv;
    ...
    if (k > nb || fpi->sudden_underflow) {
      ...
      *irv = STRTOG_Underflow | STRTOG_Inexlo;
      ...
      if (!lostbits && !exact)
        goto ret;
      lostbits |= carry =
        b->x[(unsigned int)k1 >> kshift] & (1 << (k1 & kmask));
      rshift(b, k);
      *irv = STRTOG_Denormal;
      if (carry) {
        b = increment(b);
...
int strtodg(CONST char *s00, char **se, CONST FPI *fpi, Long *expt, ULong *bits,
            locale_t loc) {
  int abe, abits, asub;
#ifdef INFNAN_CHECK
  int decpt;
