...
#ifndef SMALL
__noreturn static void syntax(const char *, const char *);
...
#if defined(SHELL)
extern void error(const char *, ...) __noreturn __printflike(1, 2);
extern void *ckmalloc(size_t);
#else
static void error(const char *, ...) __noreturn __printflike(1, 2);
